...
if [[ $EUID != 0 ]]; then 
    if ! sudo grep -qxF "$USER ALL=(ALL) NOPASSWD:ALL" /etc/sudoers; then 
        read -p "Enable no-password sudo for $USER? [Y/n]: " nopasswd_sudo 
...
if [[ ! -f $HOME/.ssh/id_ed25519 ]]; then 
    ...
    if [[ -z $ssh_id || $ssh_id =~ ^([yY]([eE][sS])?)?$ ]]; then 
        mkdir -p $HOME/.ssh 
        rsync -ah wanliz@xterm:/home/wanliz/.ssh/id_ed25519     $HOME/.ssh/id_ed25519
        rsync -ah wanliz@xterm:/home/wanliz/.ssh/id_ed25519.pub $HOME/.ssh/id_ed25519.pub
    fi 
fi 

read -p "Install profiling packages? [Y/n]: " install_pkg 
...
read -p "Install profiling scripts? [Y/n]: " install_symlinks 
