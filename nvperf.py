...
class Table_view:
    def __init__(self, columns, header=None):
        ...
        for r, row in enumerate(rows):
            ...
            right = "".join(format_cell(r, c, row[c]).ljust(columns_width[c]) for c in range(1, len(row))) if r == 0 else "".join(format_cell(r, c, row[c]).rjust(columns_width[c]) for c in range(1, len(row))) 
