...
class CMD_config:
    ...
    def config_linux_host(self):
        ...
        if mount_dirs:
            ...
            if choice: 
                mount_cmds = [f"mkdir -p {item[1]}; sudo mount -t nfs -o timeo=20,retrans=2 {item[0]} {item[1]} && echo 'Mounted {item[1]}' || echo 'Failed to mount {item[1]}'" for item in mount_dirs]
